...
function init() {
	...
	// sub camera
	subCamera = new THREE.PerspectiveCamera(70, window.innerWidth / window.innerHeight, 0.1, 4200);
	subCamera.cameraMode = CAMERA_MODE.ROBOT_VIEW;
	...
	// env
	env = new Env();
	env.initEnv(scene, camera, subCamera);
...
function animate() {	
	...
	if(!car) {
		subCamera.position.set(camera.position.x, camera.position.y, camera.position.z);
		subCamera.rotation.set(camera.rotation.x, camera.rotation.y, camera.rotation.z);		
	} else {
		switch(subCamera.cameraMode) {
			case CAMERA_MODE.ROBOT_VIEW: {
				var subCameraPos = car.directionVector.clone().multiplyScalar(car.cameraOffset);
				subCameraPos.addVectors(subCameraPos, car.mesh.position);
				subCamera.position.set(subCameraPos.x, subCameraPos.y, subCameraPos.z);
				var robotLookAt = subCameraPos.addVectors(subCameraPos, car.directionVector.clone());
				subCamera.lookAt(robotLookAt);
				break;
			}
			case CAMERA_MODE.ROBOT_HEAD_VIEW: {
				subCamera.position.set(car.mesh.position.x, car.mesh.position.y + 300, car.mesh.position.z);
				subCamera.lookAt(car.mesh.position);
				subCamera.rotation.z = car.mesh.rotation.y;
				subCamera.rotation.z += Math.PI
				break;
			}
			case CAMERA_MODE.SKY_VIEW: {
				subCamera.position.set(0, 1000, 0);
